		...
		<div className="flex w-fit items-center gap-1 rounded-full bg-white/5 p-1">
			{options.map((v) => (
				<button
					key={v}
					onClick={() => onChange(v)}
					className="rounded-full px-3 py-1 text-base font-medium transition"
					style={
						value === v
							? {
...
export function StandingsPage() {
	...
	const { data: driverStandings, isLoading: driversLoading } =
		useDriverStandings(undefined, isDrivers);
	const { data: constructorStandings, isLoading: constructorsLoading } =
		useConstructorStandings(undefined, !isDrivers);
	...
	const { data: driverEvolutionRaw, isLoading: driverEvolutionLoading } =
		useStandingsEvolution(completedRounds, isDrivers && view === 'evolution');
	const {
		data: constructorEvolutionRaw,
		isLoading: constructorEvolutionLoading,
	} = useConstructorStandingsEvolution(
		completedRounds,
	...
	const driverChartData = useMemo(() => {
		...
		return driverEvolutionRaw.map(({ round, standings: s }) => {
			...
			s.forEach((d) => {
				point[`${d.Driver.givenName[0]}. ${d.Driver.familyName}`] = Number(
					d.points,
				);
			});
	...
	const driverSeries = useMemo(() => {
		...
		return driverStandings.map((d) => ({
			...
			teamId:
				CONSTRUCTOR_ID_MAP[d.Constructors[0]?.constructorId] ?? 'mercedes',
		}));
	}, [driverStandings]);
