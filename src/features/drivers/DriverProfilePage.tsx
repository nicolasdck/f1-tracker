...
export function DriverProfilePage() {
  ...
  const { data: races, isLoading: resultsLoading } = useDriverSeasonResults(driverId ?? "");
    ...
    <div className="p-6">
      ...
      <div className="overflow-hidden rounded-lg border border-white/8">
        {races?.map((race, i) => {
          ...
          return (
            <TableRow key={race.round} index={i} highlight={isFavRow} highlightColor={theme?.primary}>
              <div className="flex items-center gap-3">
                <span className="w-5 font-mono text-white/40">{race.round}</span>
              ...
              <div className="text-right">
                ...
                <div className="font-mono text-[10px] text-white/40">{result.points} pts</div>
              </div>
            </TableRow>
          );
        })}
